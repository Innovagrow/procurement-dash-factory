# Οι σελίδες σε δικό σας droplet

Ένα droplet δίνει αυτό που δεν δίνει κανένα τοπικό αρχείο: μια διεύθυνση που
ανοίγει από παντού, χωρίς να μοιράζεστε τίποτα με κανέναν. Οι σελίδες μπαίνουν
πίσω από κωδικό και δηλώνονται `noindex`, οπότε δεν τις βρίσκει ούτε μηχανή
αναζήτησης.

**Δεν πειράζει ό,τι ήδη τρέχει εκεί.** Τα περισσότερα droplet σερβίρουν κάτι
στις θύρες 80 και 443. Το script δεν τις διεκδικεί ποτέ: πιάνει μια ελεύθερη
θύρα από το 8080 και πάνω και τυπώνει ποια βρήκε. Η διεύθυνσή σας βγαίνει
`http://ΤΟ-IP:8080/`. Με δική σας θύρα: `PORT=9000 bash /tmp/d.sh`.

## Από Windows, χωρίς να μπείτε στο droplet

> Όπου βλέπετε `203.0.113.10`, βάζετε τη **δική σας** διεύθυνση droplet —
> DigitalOcean → Droplets → στήλη **IP Address**.
>
> Η `203.0.113.10` είναι δεσμευμένη για παραδείγματα και δεν αντιστοιχεί σε
> κανένα μηχάνημα: αν επικολλήσετε την εντολή αυτούσια, απλώς δεν θα συνδεθεί.
> Αυτό είναι σκόπιμο. Μια διεύθυνση που μοιάζει αληθινή μπορεί να **είναι**
> αληθινή και να ανήκει σε άλλον — και τότε το `ssh` ρωτάει αν εμπιστεύεστε το
> κλειδί ενός ξένου διακομιστή. Αν δείτε τέτοια ερώτηση και δεν αναγνωρίζετε τη
> διεύθυνση, απαντήστε `no`.

Τα Windows 10 και 11 έχουν ήδη `ssh`. Από **cmd**, μία γραμμή:

```
ssh root@203.0.113.10 "curl -fsSL https://raw.githubusercontent.com/Innovagrow/procurement-dash-factory/claude/greek-brokers-investment-outreach-gr5j9p/deploy/droplet.sh -o /tmp/d.sh && bash /tmp/d.sh"
```

Την πρώτη φορά το `ssh` ρωτάει αν εμπιστεύεστε το κλειδί του διακομιστή:
γράψτε `yes`. Μετά ζητάει τον κωδικό του droplet, αν δεν χρησιμοποιείτε κλειδί.

Ή **διπλό κλικ στο `deploy\DROPLET.bat`**: ρωτάει διεύθυνση, χρήστη και
προαιρετικό domain, στήνει το droplet και προσφέρεται να ανεβάσει και την
τελευταία αναφορά σάρωσης.

Αν το `ssh` δεν αναγνωρίζεται: Ρυθμίσεις → Σύστημα → Προαιρετικές δυνατότητες →
Προσθήκη → **OpenSSH Client**.

## Μία εντολή, πάνω στο droplet

Συνδεθείτε στο droplet — από την κονσόλα του DigitalOcean ή με
`ssh root@203.0.113.10` — και επικολλήστε:

```
curl -fsSL https://raw.githubusercontent.com/Innovagrow/procurement-dash-factory/claude/greek-brokers-investment-outreach-gr5j9p/deploy/droplet.sh | bash
```

Στο τέλος τυπώνει τη διεύθυνση, τον χρήστη και τον κωδικό. Ο κωδικός φυλάσσεται
και στο `/root/akinita-kwdikos.txt`.

Αν προτιμάτε να δείτε πρώτα τι θα τρέξει — καλή συνήθεια για ό,τι κατεβάζετε:

```
curl -fsSLO https://raw.githubusercontent.com/Innovagrow/procurement-dash-factory/claude/greek-brokers-investment-outreach-gr5j9p/deploy/droplet.sh
less droplet.sh
bash droplet.sh
```

Αν θέλετε τις σελίδες κάτω από δικό σας domain με `https`, μην αλλάξετε αυτό το
script: προσθέστε μια διαδρομή στον reverse proxy που ήδη τρέχει στο μηχάνημα,
δείχνοντας στο `http://127.0.0.1:8080`. Έτσι κρατάτε το πιστοποιητικό που ήδη
έχετε, αντί να διεκδικήσετε τη θύρα 443 από την υπηρεσία που την κρατά.

Ξανατρέξτε το όποτε θέλετε. Κάνει `pull`, ξαναφτιάχνει τις σελίδες και αφήνει
τον κωδικό όπως είναι.

## Τι στήνεται

**Μία σελίδα, με καρτέλες.** Η «Πού να ψάξεις» γεμίζει μόνη της από ανοιχτά
δεδομένα και ανανεώνεται καθημερινά. Η «Ευκαιρίες» γεμίζει από τη σάρωση που
τρέχετε στον υπολογιστή σας και ανεβάζετε ως ένα αρχείο.

| | |
|---|---|
| `/` | Η σελίδα — ευκαιρίες, περιοχές, και πώς διαβάζονται |
| `/scan.json` | Η σάρωσή σας· το ανεβάζετε εσείς |
| `/simata.json` | Τα ωμά δεδομένα των περιοχών |

Η καθημερινή ανανέωση είναι systemd timer. Έλεγχος:

```
systemctl list-timers akinita.timer
journalctl -u akinita.service -n 30
```

## Η σάρωση αγγελιών ΔΕΝ τρέχει στο droplet

Αυτό είναι το σημείο που κοστίζει χρόνο αν δεν το ξέρετε από πριν.

Δύο εμπόδια, και τα δύο επιβεβαιωμένα με δοκιμή:

1. Οι πύλες απαντούν σε αυτοματοποιημένες συνεδρίες με **hCaptcha** — κέλυφος
   λίγων χιλιάδων bytes αντί για αποτελέσματα. Δεν παρακάμπτεται και δεν
   επιχειρείται.
2. Πολλές διευθύνσεις data center απορρίπτονται και στο CDN με `403`.

Το `droplet.sh` το ελέγχει μόνο του με πραγματικό browser και ενεργοποιεί τη
σάρωση **μόνο** αν όντως επιστρέφονται αγγελίες. Αλλιώς την αφήνει σβηστή και
το λέει. Ό,τι κι αν γίνει, η καρτέλα «Πού να ψάξεις» δουλεύει: στηρίζεται σε
ανοιχτά δεδομένα που δεν μπλοκάρει κανείς.

Άρα ο καταμερισμός είναι:

* **Σάρωση** στον υπολογιστή σας, από οικιακή σύνδεση.
* **Δημοσίευση** στο droplet, για να τη βλέπετε από παντού.

Ανεβάστε την αναφορά μετά τη σάρωση, από γραμμή εντολών των Windows:

```
scp out\eukairies_analysis.json root@203.0.113.10:/var/www/akinita/scan.json
```

Μετά το ανέβασμα, ξανατρέξτε το script μία φορά για να ξαναχτιστεί η σελίδα —
ή αφήστε το να γίνει μόνο του με την επόμενη καθημερινή ανανέωση.

Το `scp` υπάρχει ήδη σε Windows 10 και 11.

## Ό,τι ανεβαίνει, μένει δικό σας

Οι Όροι των πυλών επιτρέπουν την προσωπική χρήση των δεδομένων και απαγορεύουν
την αναδημοσίευση. Ένας ιστότοπος με κωδικό, που τον βλέπετε μόνο εσείς, δεν
είναι δημοσίευση — ένας ανοιχτός ιστότοπος είναι. Γι' αυτό το `auth_basic` δεν
είναι προαιρετικό εδώ, και γι' αυτό ο χάρτης, που στηρίζεται αποκλειστικά σε
ανοιχτά δεδομένα, είναι το μόνο κομμάτι που θα μπορούσε να είναι δημόσιο.

## Αν η σελίδα δεν ανοίγει

Με τη σειρά:

1. **Cloud firewall του DigitalOcean** — Networking → Firewalls → Inbound Rules →
   επιτρέψτε `TCP 8080`. Είναι έξω από το droplet, οπότε το `ufw` δεν το βλέπει.
2. **ufw πάνω στο droplet** — το script ανοίγει μόνο του τη θύρα αν το ufw είναι
   ενεργό. Έλεγχος: `ufw status`.
3. **nginx** — `systemctl status nginx` και `nginx -t`.
4. **Θύρα** — `ss -ltn | grep 8080` δείχνει αν ακούει κανείς εκεί.
